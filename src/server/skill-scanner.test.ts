...
test('detects the generation version when only the Command Code skill root exists', async () => {
  const root = await makeProjectRoot();
  await write(root, '.commandcode/skills/openspec-propose/SKILL.md', skillMarkdown('1.9.0'));

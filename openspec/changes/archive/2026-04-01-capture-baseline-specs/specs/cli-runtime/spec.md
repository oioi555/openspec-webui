...
The system SHALL accept an OpenSpec-compatible directory path and port from the CLI, default the path to the current working directory, default the port to `3000`, bind to `127.0.0.1` unless configured otherwise, and reject paths that do not exist or are not directories.
...
- **AND** the local server listens on port `3000`

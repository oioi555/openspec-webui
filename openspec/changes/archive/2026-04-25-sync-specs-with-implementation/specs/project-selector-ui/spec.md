# project-selector-ui Delta Specification
...
The web UI SHALL render a dedicated AddProjectDialog component when `layoutStore.overlay` is set to `'add-project'`. The dialog SHALL prioritize directory browsing, SHALL show which subdirectories contain an `openspec/` folder, and SHALL provide a manual path fallback. Selecting the current directory or submitting a manual path SHALL add and activate that project. The manual path fallback SHALL accept any non-empty string without absolute-path validation; the server validates the path.

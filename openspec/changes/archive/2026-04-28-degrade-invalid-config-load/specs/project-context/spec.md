...
#### Scenario: Load project planning context from valid config.yaml
- **WHEN** the target OpenSpec directory contains a readable and valid `config.yaml`
...
#### Scenario: Expose invalid planning context from malformed config.yaml
- **WHEN** the target OpenSpec directory contains a readable `config.yaml`
- **AND** the YAML content cannot be parsed
- **THEN** the system still exposes the derived project name
- **AND** returns a planning context marked as invalid
- **AND** includes parse error details and the raw `config.yaml` text
- **AND** does not synthesize `AI Context`, `Artifact Rules`, or `Workflow Schema` from repaired content

#### Scenario: Derive the short description from AI Context
- **WHEN** the target OpenSpec directory contains a non-empty `context:` block in valid `config.yaml`
- **THEN** the system derives the project description from the first non-empty line of that block

#### Scenario: Avoid misleading description when config is invalid
- **WHEN** the target OpenSpec directory contains malformed `config.yaml`
- **THEN** the system does not derive the project description from partially parsed YAML content
